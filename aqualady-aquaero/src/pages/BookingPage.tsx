...
import { loadPools, PRICES, type PoolId } from '../config'

export default function BookingPage() {
  const navigate = useNavigate()
  const { dispatch, state: cartState } = useCart()

  const [poolList] = useState(() => Object.values(loadPools()))
  ...
  // Count booked slots per pool/date/time from cart
  ...
  const addSingleSession = () => {
    ...
    const id = `single-${selectedPool}-${selectedDate}-${selectedTime}-${Date.now()}-${Math.random()}`
    ...
    dispatch({
      ...
      payload: {
        ...
        poolId: selectedPool,
        type: 'single',
        label: `Zajęcia ${timeLabel} — ${selectedDate.slice(8, 10)}.${selectedDate.slice(5, 7)}`,
        date: selectedDate,
        time: selectedTime,
        price: PRICES.single,
        quantity: 1,
    ...
    showAnimation('Zajęcia zostały dodane do koszyka!')
    setSelectedDate(null)
  }
  ...
  const addPass = (type: 'pass8' | 'pass12' | 'pass16') => {
    ...
    dispatch({
      ...
      payload: {
        ...
        poolId: selectedPool,
        type,
        label: `${label} — ${poolList.find(p => p.id.toUpperCase() === selectedPool)?.name || selectedPool}`,
        price: PRICES[type],
        quantity: 1,
      },
    })
  ...
  const currentPool = selectedPool ? poolList.find(p => p.id.toUpperCase() === selectedPool) : null
      ...
      {selectedPool && (
        <div className="space-y-4 mt-5 pt-5 border-t border-sand/20">
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => { setSelectedPool(null); setExpandedMap(null); setSelectedDate(null); }}
          ...
          {/* Booked info */}
          {selectedDate && selectedPool && (bookedCount.get(selectedPool + '|' + selectedDate + '|' + selectedTime) || 0) > 0 && (
            <p className="text-[10px] text-amber-600">
              ⚠️ Ten termin może być już częściowo zajęty.
            </p>
          )}
          ...
          <div className="space-y-3">
            <button
              ...
              className={`w-full py-3.5 rounded-xl font-bold text-sm transition-all ${!selectedDate
                  ? 'bg-stone-200 text-stone-400 cursor-not-allowed'
                  : 'bg-teal-brand text-white shadow-lg hover:bg-teal-light active:scale-[0.98]'
              }`}
            >
              Dodaj zajęcia · {PRICES.single} zł
            </button>
